...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        if (
            ...
            msg += f"╭ <a href='{task.listener.message.link if task.listener.is_super_chat else ''}'>{tstatus} » {progress}</a>"
            ...
            msg += f"\n┊ <b>ETA:</b> {task.eta()}"
            msg += f"\n┊ <b>Past:</b> {get_readable_time(time() - task.listener.message.date.timestamp() + get_raw_time(task.eta()))}"
            msg += f"\n┊ <b>Engine:</b> {task.engine}"
            msg += f"\n┊ <b>Mode:</b> {task.listener.mode[1]}\n"
